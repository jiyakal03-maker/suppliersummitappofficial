...
import { GM_ALT_BUILDER_ONBOARDING_SEEN_KEY, GM_ALT_SPECTATOR_ONBOARDING_SEEN_KEY } from "@/lib/onboarding";

/**
 * Same spotlight-coachmark pattern as components/board-onboarding-tour.tsx
 * (the tldraw version), adapted for /gm-alt. Only targets elements we
 * render ourselves (prompt banner, Leave board) via `data-tour` — unlike
 * tldraw, Excalidraw doesn't offer a `components` override to wrap and tag
 * its own toolbar/panel internals, so the toolbar step here has no
 * specific target (Excalidraw's toolbar sits on the left by default).
 */
...
const BUILDER_STEPS: Step[] = [
  ...
  },
  {
    selector: null,
    title: "Draw here",
    body: "Use the toolbar on the left to sketch your answer to the prompt.",
  },
  {
...
export function BoardOnboardingTour({ role }: { role: "builder" | "spectator" }) {
  const seenKey = role === "builder" ? GM_ALT_BUILDER_ONBOARDING_SEEN_KEY : GM_ALT_SPECTATOR_ONBOARDING_SEEN_KEY;
  ...
  const tooltipStyle: React.CSSProperties = rect
    ? (() => {
        ...
        const top = placeAbove ? Math.max(rect.top - TOOLTIP_HEIGHT_ESTIMATE - 16, MARGIN) : rect.bottom + 16;
    ...
    <div className="pointer-events-none fixed inset-0 z-[700]">
      ...
      >
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <Button variant="contained" color="primary" onClick={() => (isLast ? finish() : setIndex((i) => i + 1))}>
            {isLast ? "Done" : "Next"}
          </Button>
        </div>
      </div>
